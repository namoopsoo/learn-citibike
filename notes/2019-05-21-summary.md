...
#### Later on,
* Maybe a year or more later I came back to this problem from the point of view of experimenting with [AWS approaches to training models](#sagemaker-approach)
* SageMaker, Docker, more compartmentalized model, and an approach that can make model iteration perhaps better.
* this time around, there are `65` neighborhoods in the data . 
* This time around I also wanted to have a slightly better time debugging [changes my model iterations](#changes-to-model-iterations)
...
* I had a top of mind goal to try and explore model degradation over time. I stuck with one `RandomForestClassifier` classifier, (defined [here](https://github.com/namoopsoo/learn-citibike/blob/master/bikelearn/models/treefoo.py)) , 
